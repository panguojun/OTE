...
void CEntEditDlg::OnTvnSelchangedTree1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if(!txt.IsEmpty())
	{
		CMeshEditorDlg::s_Inst.CancelSelection();
		
		if(txt.Find("Ent_") != -1)
			CMeshEditorDlg::s_Inst.SelectSceneEntity(SCENE_MGR->SelectEntity((LPCTSTR)txt, true));
		else if((txt.Find("Mag_") != -1))
			CMeshEditorDlg::s_Inst.SelectSceneEntity(MAGIC_MGR->GetMagic(txt.GetString()));
		
		if(!CMeshEditorDlg::s_pSelGroup.empty())
		{
			(*CMeshEditorDlg::s_pSelGroup.begin())->getParentSceneNode()->showBoundingBox(true);
			CEntPorpDlg::sInst.UpdateEntityPropertyList((*CMeshEditorDlg::s_pSelGroup.begin()));
			CMeshEditorDlg::s_TransMgr.SetDummyPos((*CMeshEditorDlg::s_pSelGroup.begin())->getParentNode()->getPosition());
		}		
		
		// 摄像机

		OTE::COTEActorEntity* ent = ((OTE::COTEActorEntity*)(*CMeshEditorDlg::s_pSelGroup.begin()));
...
{
	CMeshEditorDlg::s_Inst.RemoveSelEntities();
