...
//Read File TGA
//******************************************************
int CTexture::ReadFileTGA(const char* filename)
{
		 ...
		 (header[4]!=24 && header[4]!=32))										// Is The TGA 24 or 32 Bit?
    {
        ...
        if (m_Width  <=0 || m_Height  <=0)
            TRACE("Image must have a width and height greater than 0","Error",MB_OK|MB_ICONERROR);
        if (header[4]!=24 && header[4]!=32)
            TRACE("Image must be 24 or 32 bit","Error",MB_OK|MB_ICONERROR);

		fclose(file);
        return 0;                                               // Return False
    }
    m_Depth			= header[4];                                        // Grab The TGA's Bits Per Pixel (24 or 32)
    bytesPerPixel   = m_Depth/8;                                   // Divide By 8 To Get The Bytes Per Pixel
    imageSize       = m_Width*m_Height*bytesPerPixel;     // Calculate The Memory Required For The TGA Data
	m_pData			= new unsigned char [imageSize]; 

    if (m_pData == NULL ||                                     // Does The Storage Memory Exist?
    fread(m_pData, 1, imageSize, file)!=imageSize)           // Does The Image Size Match The Memory Reserved?
    {
        if (m_pData!=NULL)                                    // Was Image Data Loaded
            free(m_pData);                                   // If So, Release The Image Data
        TRACE("Image load failed for unknown reason.","Error",MB_OK|MB_ICONERROR);
...
int CTexture::SaveFileBMP(const char *filename)
{
    ...
    if(m_Depth !=24)
	{
	  if(m_Depth == 32)//delete alpha layer first
	  {
		if(!DeleteAlpha())
		  return 0;
	  }
...
//save as tga
//********************************************
bool CTexture::SaveAsTga(const char *filename)
{
	if(!IsValid())
		return 0;
	UpdateHeader();
...
//		return 0;
	...
	if(!file.Open(filename,CFile::modeCreate | CFile::modeWrite | CFile::typeBinary,&ex))
	{
	    ...
	    unsigned char TGAheader[12]={0,0,2,0,0,0,0,0,0,0,0,0};                        // Uncompressed TGA Header
        file.Write(TGAheader,sizeof(TGAheader));//TGA Header

          unsigned char header[6];	
          header[1]=m_Width/256;
		  header[0]= m_Width%256;
          header[3]=m_Height/256;
		  header[2]= m_Height%256;
		  header[4]=m_Depth ;  // Is The TGA 24 or 32 Bit?
 		  header[5]=0;//?
...
int CTexture::AddAlphaLayer(unsigned char alpha) // 0 - 255
{
	...
	for(int i=0;i<size;i++)
	{
		...
		if(pData[4*i+0]==0 && pData[4*i+1]==0 && pData[4*i+2]==0)
		    pData[4*i+3] = 0;
		else
			pData[4*i+3] =alpha;
	}
...
int CTexture::DeleteAlpha()
{
	...
	if(pData == NULL)
		{
		AfxMessageBox("CTexture::DeleteAlpha : insuffisant memory");
		return 0;
		}
	...
	for(int i=0;i<size;i++)
	{
		pData[3*i+0] = m_pData[BytePerPixel*i+0];
		pData[3*i+1] = m_pData[BytePerPixel*i+1];
		pData[3*i+2] = m_pData[BytePerPixel*i+2];	
	}
...
//****************************************************
//Set Alpha by color index
//****************************************************
bool CTexture::SetAlphaByColIndex(unsigned char color[],unsigned char alpha)
{
  	// Check
	//if(!IsValid())
	//	return 0;
